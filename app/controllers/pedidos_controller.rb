    ...
    def create 
       ...
       if @pedido.save && @pedido.fecha_de_entrega >= (Date.today + 2)
            if @pedido.fecha_de_entrega.strftime("%A") == "Monday"
                redirect_to new_torta_path(parametro1: @pedido.a_nombre_de), notice: "Los pedidos se pueden retirar o enviar de martes a domingo, tu pedido va a estar listo el martes"
                @pedido.fecha_de_entrega+=1
                @pedido.save
            else
                redirect_to new_torta_path(parametro1: @pedido.a_nombre_de), notice: "Se guardo el pedido correctamentee"
            end
       else
